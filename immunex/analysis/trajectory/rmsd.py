...
class RMSDCalculator:
    ...
    def calculate_mdanalysis(self,
        ...
        for ts in self.universe.trajectory:
            # MDAnalysis 返回单位为 Angstrom，这里统一转换为 nm。
    ...
    def calculate_with_index_manager(self,
                                    context: 'PipelineContext',
                                    fit_component: str,
                                    calc_component: str,
                                    output_file: Optional[str] = None) -> str:
        """
        Calculate RMSD using IndexManager for group selection.

        This method integrates with the unified IndexManager to:
        1. Automatically obtain group IDs from component names
        2. Use the correct index file
        3. Perform selective fit RMSD (align on fit_component, calc on calc_component)

        Args:
            context: PipelineContext instance containing IndexManager
            fit_component: Component name for alignment (e.g., 'pHLA', 'HLA')
            calc_component: Component name for RMSD calculation (e.g., 'TCR', 'peptide')
        ...
        # Get group IDs
        fit_group_id = index_mgr.get_group_id(fit_component)
        calc_group_id = index_mgr.get_group_id(calc_component)

        if fit_group_id is None:
            raise ValueError(f"Component '{fit_component}' not found in index registry")
        if calc_group_id is None:
            raise ValueError(f"Component '{calc_component}' not found in index registry")

        # Get index file
        index_file = index_mgr.get_combined_index(fit_component, calc_component)

        # Generate output file if not specified
        if output_file is None:
            output_file = context.get_analysis_path(
                "rmsd",
                f"rmsd_{fit_component}_fit_{calc_component}_calc.xvg"
            )

        logger.info(f"RMSD calculation with IndexManager:")
        logger.info(f"  Fit component:  {fit_component} (group {fit_group_id})")
        logger.info(f"  Calc component: {calc_component} (group {calc_group_id})")
        logger.info(f"  Index file:     {index_file.name}")

        # Construct GROMACS command
        stdin_input = f"{fit_group_id}\n{calc_group_id}\n"
        ...
        try:
            ...
            logger.info(f"RMSD calculation completed: {output_file}")
            return output_file
        except subprocess.CalledProcessError as e:
            logger.error(f"GROMACS RMSD calculation failed: {e}")
            logger.error(f"stderr: {e.stderr}")
    ...
    def _write_xvg_file(self, filename: str, times: np.ndarray, rmsd_values: np.ndarray):
        ...
        with open(filename, 'w') as f:
            f.write("# RMSD calculation by Immunex\n")
