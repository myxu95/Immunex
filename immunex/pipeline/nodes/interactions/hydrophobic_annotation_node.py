...
class HydrophobicAnnotationNode(PipelineNode):
    """为 residue-level 疏水接触结果添加语义注释并写出区域视图。"""

    def __init__(self, name: str | None = None):
        super().__init__(name=name or "HydrophobicAnnotationNode")

    def validate_inputs(self, context: PipelineContext):
        if "hydrophobic_pairs" not in context.results:
            raise PipelineError(self.name, "hydrophobic_pairs results not found in context", {"system_id": context.system_id})
    ...
    def execute(self, context: PipelineContext) -> PipelineContext:
        ...
        raw_file = context.results["hydrophobic_pairs"]["raw_pairs_file"]
        ...
        output_dir = Path(context.get_analysis_path("interactions/hydrophobic_contacts", "annotated_hydrophobic_pairs.csv")).parent
        output_dir.mkdir(parents=True, exist_ok=True)
        annotated_file = str(output_dir / "annotated_hydrophobic_internal.csv")
        annotated.to_csv(annotated_file, index=False)
        report_table = ContactAnnotationNode._build_contact_report_table(annotated)
        report_file = str(output_dir / "hydrophobic_report.csv")
        ...
        groove_file = str(output_dir / "groove_tcr_hydrophobic_contacts.csv")
        ...
        summary_file = str(output_dir / "hydrophobic_annotation_summary.json")
        ...
        context.results["hydrophobic_annotation"] = {
            "annotated_pairs_file": annotated_file,
            "hydrophobic_report_file": report_file,
            "groove_tcr_hydrophobic_contacts_file": groove_file,
