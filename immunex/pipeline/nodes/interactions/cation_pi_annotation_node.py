...
class CationPiAnnotationNode(PipelineNode):
    """为 residue-level cation-pi 结果添加语义注释并写出区域视图。"""

    def __init__(self, name: str | None = None):
        super().__init__(name=name or "CationPiAnnotationNode")

    def validate_inputs(self, context: PipelineContext):
        if "cation_pi_pairs" not in context.results:
            raise PipelineError(self.name, "cation_pi_pairs results not found in context", {"system_id": context.system_id})
    ...
    def execute(self, context: PipelineContext) -> PipelineContext:
        ...
        raw_file = context.results["cation_pi_pairs"]["raw_pairs_file"]
        ...
        output_dir = Path(context.get_analysis_path("interactions/cation_pi_interactions", "annotated_cation_pi_pairs.csv")).parent
        output_dir.mkdir(parents=True, exist_ok=True)
        annotated_file = str(output_dir / "annotated_cation_pi_internal.csv")
        annotated.to_csv(annotated_file, index=False)
        report_table = ContactAnnotationNode._build_contact_report_table(annotated)
        report_file = str(output_dir / "cation_pi_report.csv")
        ...
        groove_file = str(output_dir / "groove_tcr_cation_pi.csv")
        ...
        summary_file = str(output_dir / "cation_pi_annotation_summary.json")
        ...
        context.results["cation_pi_annotation"] = {
            "annotated_pairs_file": annotated_file,
            "cation_pi_report_file": report_file,
            "groove_tcr_cation_pi_file": groove_file,
