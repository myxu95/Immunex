...
class SaltBridgeAnnotationNode(PipelineNode):
    """为 residue-level 盐桥结果添加语义注释并写出区域视图。"""

    def __init__(self, name: str | None = None):
        super().__init__(name=name or "SaltBridgeAnnotationNode")

    def validate_inputs(self, context: PipelineContext):
        if "salt_bridge_pairs" not in context.results:
            raise PipelineError(self.name, "salt_bridge_pairs results not found in context", {"system_id": context.system_id})
    ...
    def execute(self, context: PipelineContext) -> PipelineContext:
        ...
        raw_file = context.results["salt_bridge_pairs"]["raw_pairs_file"]
        ...
        output_dir = Path(context.get_analysis_path("interactions/salt_bridges", "annotated_salt_bridge_pairs.csv")).parent
        output_dir.mkdir(parents=True, exist_ok=True)
        annotated_file = str(output_dir / "annotated_salt_bridge_internal.csv")
        annotated.to_csv(annotated_file, index=False)
        report_table = ContactAnnotationNode._build_contact_report_table(annotated)
        report_file = str(output_dir / "salt_bridge_report.csv")
        ...
        groove_file = str(output_dir / "groove_tcr_salt_bridges.csv")
        ...
        summary_file = str(output_dir / "salt_bridge_annotation_summary.json")
        ...
        context.results["salt_bridge_annotation"] = {
            "annotated_pairs_file": annotated_file,
            "salt_bridge_report_file": report_file,
            "groove_tcr_salt_bridges_file": groove_file,
